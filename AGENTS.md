...
4. **Bonuses** — all four optional challenges are implemented: **B1 (alerting)**, **B2 (Kafka streaming ingestion)**, **B3 (time-series)**, **B4 (rate limiting)**. B2 adds a Kafka ingestion entry point (topic `events.ingest`) alongside REST; B1/B3/B4 are analytics-side. See the README's "Bonus features".
